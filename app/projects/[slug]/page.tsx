...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const cleanDescription = project.story.substring(0, 155).trim() + "...";
  return {
    title: `${project.title} | HUED`,
    description: cleanDescription,
    openGraph: {
      title: `${project.title} — HUED`,
      description: cleanDescription,
      url: `https://huedspaces.com/projects/${project.slug}`,
      siteName: "HUED",
      images: [
        {
  ...
  return (
    <div className="bg-bone min-h-screen pb-20">
      ...
      <section className="relative h-[95vh] w-full flex items-end justify-start px-6 md:px-12 pb-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full flex flex-col items-start animate-[fadeIn_1.2s_ease-out]">
          <div>
          ...
          {/* Spatial Specs Strip */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-12 mt-12 w-full max-w-4xl border-t border-bone/20 pt-8">
            <div>
      ...
      <section className="py-28 md:py-36 px-6 md:px-12 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
        <div className="lg:col-span-5">
          ...
          <h3 className="font-serif text-3xl md:text-4xl font-light text-ink leading-tight tracking-tight max-w-sm">
            {project.subtitle}
          </h3>
          ...
          <div className="flex flex-wrap gap-2 mt-6">
            {project.materials.map((m) => (
              <span 
                key={m} 
                className="font-sans text-[10px] uppercase tracking-widest bg-sand/35 border border-stone/10 px-3 py-1 text-ink/70 rounded-full"
              >
                {m}
              </span>
            ))}
        ...
        <div className="lg:col-span-7 flex flex-col justify-center">
          <p className="font-sans text-lg md:text-xl font-light leading-relaxed text-ink/70 mb-8">
            {project.story}
          </p>
            ...
            {project.images.slice(1).map((img, idx) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <div className="absolute bottom-4 left-4 bg-ink/60 backdrop-blur-sm px-3 py-1 font-sans text-[9px] uppercase tracking-widest text-bone opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    Figure 0{idx + 2}
                  </div>
                </div>
              );
      ...
      <section className="py-28 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          <div className="lg:col-span-5">
            ...
            <h3 className="font-serif text-3xl md:text-4xl font-light text-ink tracking-tight mb-6">
              Tactile Specifications
            </h3>
            <p className="font-sans text-sm font-light text-ink/60 leading-relaxed max-w-sm mb-8">
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {project.materials.map((mat, index) => (
              <div 
                key={mat} 
                className="bg-sand/20 border border-stone/10 p-6 rounded-none flex items-start space-x-4 group hover:bg-terracotta/5 transition-colors duration-500 rounded-sm"
              >
                ...
                <div>
                  <h4 className="font-serif text-lg text-ink mb-1">{mat}</h4>
      ...
      <section className="border-t border-stone/20 pt-20 mt-12 bg-sand/5">
        <div className="max-w-7xl mx-auto px-6 md:px-12 text-center py-20 flex flex-col items-center">
          ...
          >
            <h3 className="font-serif text-4xl sm:text-6xl md:text-7xl font-light text-ink group-hover:text-terracotta transition-colors duration-500 leading-tight">
              {nextProject.title}
            </h3>
